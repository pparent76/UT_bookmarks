...
MainView {
  ...
  property var appID: "tandem.pparent";
  property var hook: "tandem";  
  property var localStorage: "/home/phablet/.cache/tandem.pparent/QtWebEngine";  
  ...
  Settings {
        ...
        property int webviewWidthPortait: 600
 ...
 DownloadHelper {
        id: downloadHelper
        blob_path: localStorage+"/IndexedDB/https_web.tandem.com_0.indexeddb.blob/"
    }   
  ...
  PageStack {
    ...
    Page {
      ...
      WebEngineView {
        ...
        const jsCode = `document.querySelector('[class^="styles_defaultDrawer"]').style.paddingBottom = "${realKeyboardSize}px"`;
       ...
       url: "https://tandem.net/"
        ...
        onTitleChanged: {
            ...
            if ( unread>lastUnreadCount && unread>0  )
            {
               if ( config.enableTitleChangeNotifications )
                  notifier.triggerDelayedNotification2(unread+" tandem message unread");
            }
        ...
        onJavaScriptConsoleMessage: function(level, message, line, sourceId) {
          ...
          //---------------------------------------------------------------------------
            if (message.startsWith("[DbgAud] https://static.tandem.net/")) {
                //Send notification
                if ( config.enableSoundNotifications )
                  notifier.triggerDelayedNotification1("New Tandem audio notification");
            }
